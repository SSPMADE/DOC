...
.esc_pressed:				; Set carry flag if Escape was pressed
	...
	.help_msg1	db 'Select', 0
	.help_msg2	db 'Use Up, Down or Enter key', 0
...
; ------------------------------------------------------------------
; os_app_selector -- Show an app selection dialog
; IN: Nothing; OUT: AX = location of filename string (or carry set if Esc pressed)
...
.esc_pressed:				; Set carry flag if Escape was pressed
	...
	.helpssp_msg1	db 'SSPAI0', 0
	.helpssp_msg2	db 'Use Up, Down or Enter key', 0

	.filename	times 13 db 0


; ------------------------------------------------------------------Apps and files list select
...
;colors
;% BLACK_ON_WHITE		11110000b
;% WHITE_ON_BLACK		00001111b
;% WHITE_ON_LIGHT_RED	11001111b
;% WHITE_ON_GREEN		00101111b
;% RED_ON_LIGHT_GREEN	10100100b
;% White on red 01001111b
	;mov bl, 10100100b		; research on colors  syntax is background code then text code
	;mov bl, 11110000b		;ssp001 edit BLACK_ON_WHITE  syntax is background code then text code
	mov bl, 01110000b		;ssp001 edit black text on gray bg syntax is background code then text code
...
; ------------------------------------------------------------------
;SSP001 environment dialog box
ssp001_environment_dialog_box:
	pusha
...
.redbox:				; Loop to draw all lines of box
	...
	mov cx, 42
;	mov bl, 01001111b		; White on red
	mov bl, 01110000b		; black on gray ssp001 edit
	mov al, ' '
	int 10h
	popa
...
.two_button:
	...
	mov dh, 14
	call os_move_cursor
	mov si, .sspcancel_button_string
	call os_print_string
...
.two_button_wait:
	...
	mov bl, 11110000b		; [sspedit] Black on white (color of GUI button when Left cursor key pressed) (Two button start dialog for interface)
	mov dh, 14
	mov dl, 27
	mov si, 8
	mov di, 15
	...
	;mov bl, 01001111b		; White on red for CMD button
	mov bl, 00001111b		; [sspedit] white on black (color of CMD button when Left cursor key pressed) (Two button start dialog for interface)
	mov dh, 14
	mov dl, 42
	mov si, 9
	mov di, 15
...
.noleft:
	...
	;mov bl, 01001111b		; White on red for GUI button
	mov bl, 00001111b		; [sspedit] white on black (color of GUI button when Right cursor key pressed) (Two button start dialog for interface)
	mov dh, 14
	mov dl, 27
	mov si, 8
	mov di, 15
	...
	;mov bl, 11110100b
	mov bl, 11110000b		; black on white for cancel button
	mov dh, 14
	mov dl, 43
	mov si, 8
	mov di, 15
...
.noright:
	...
	.sspok_button_string	db 'GUI', 0
	.sspcancel_button_string	db 'CMD', 0
	.tmp dw 0
...
.redbox:				; Loop to draw all lines of box
	...
	mov cx, 42
;	mov bl, 01001111b		; White on red
	;mov bl, 00001111b		; white on black
	mov bl, 01110000b		;ssp001 edit black text on gray bg syntax is background code then text code
	mov al, ' '
	int 10h
	popa
...
.noright:
	...
	mov [.tmp1], cx			; Keep result after restoring all regs
	popa
	mov ax, [.tmp1]
	...
	.sspok_button db 'ok', 0
	.sspcancel_button db 'cancel', 0
	.tmp1 dw 0
